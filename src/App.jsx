...
const APIS = [
  ...
  {id:"gemini-flash",name:"Gemini 2.5 Flash",badge:"Free",type:"google",
   model:"gemini-2.5-flash",color:"#fbbf24",
   keyUrl:"https://aistudio.google.com/apikey",
   desc:"Google's fastest model. Huge context window, great reasoning."},
  {id:"gemini-pro",name:"Gemini 2.5 Pro",badge:"OpenRouter",type:"openrouter",
   model:"google/gemini-2.5-pro",color:"#fb923c",
  ...
  {id:"llama4",name:"Llama 4 Maverick",badge:"Free",type:"openrouter",
   model:"meta-llama/llama-4-maverick:free",color:"#60a5fa",
   keyUrl:"https://openrouter.ai/keys",
   desc:"Meta's latest Llama 4 model. Great reasoning and coding via OpenRouter."},
...
/* ── Themes ── */
const THEMES = [
  {id:"nexus",    name:"Nexus",       type:"dark",  dot:["#7c3aed","#6366f1"]},
  {id:"midnight", name:"Midnight",    type:"dark",  dot:["#06b6d4","#3b82f6"]},
  {id:"aurora",   name:"Aurora",      type:"dark",  dot:["#10b981","#06b6d4"]},
  {id:"sunset",   name:"Sunset",      type:"dark",  dot:["#f97316","#ef4444"]},
  {id:"ocean",    name:"Ocean",       type:"dark",  dot:["#0ea5e9","#818cf8"]},
  {id:"rose",     name:"Rose",        type:"dark",  dot:["#ec4899","#a855f7"]},
  {id:"neon",     name:"Neon",        type:"dark",  dot:["#39ff14","#ff00ff"]},
  {id:"forest",   name:"Forest",      type:"dark",  dot:["#22c55e","#16a34a"]},
  {id:"cyberpunk",name:"Cyberpunk",   type:"dark",  dot:["#facc15","#f97316"]},
  {id:"light",    name:"Light",       type:"light", dot:["#6d28d9","#4f46e5"]},
  {id:"warm",     name:"Warm Light",  type:"light", dot:["#d97706","#f59e0b"]},
  {id:"candy",    name:"Candy",       type:"light", dot:["#ec4899","#a855f7"]},
];
...
const DEF = {
  ...
  personality:"assistant",temperature:0.7,maxTokens:2000,tavilyKey:"",sysOverride:"",theme:"nexus",
};
    ...
    <div className="ov" onClick={e=>e.target.className==="ov"&&onClose()}>
      <div className="modal">
        ...
        <div className="m-tabs">
          {[["api","Models"],["personality","Personality"],["search","Search"],["themes","Themes"],["advanced","Advanced"]].map(([id,lbl])=>(
        ...
        <div className="m-body">
          ...
          {tab==="themes"&&<>
            <div className="s-sec">
              <div className="s-lbl">Theme</div>
              <div className="s-desc">Choose a look for the entire app. Saved automatically.</div>
              <div className="theme-grid">
                {THEMES.map(t=>(
                  <div key={t.id} className={`theme-card ${settings.theme===t.id?"sel":""}`} onClick={()=>upd("theme",t.id)}>
                    <div className="theme-preview">
                      <div className="tp-dot" style={{background:t.dot[0]}}/>
                      <div className="tp-dot" style={{background:t.dot[1]}}/>
                    </div>
                    <div className="theme-name">{t.name}</div>
                    {settings.theme===t.id&&<div className="theme-chk"><ICheck /></div>}
                  </div>
                ))}
              </div>
            </div>
...
function parseMarkdown(text) {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith('```')) {
      ...
      elements.push(
        <div key={key++} className="md-code-wrap">
          <div className="md-code-hdr"><span className="md-code-lang">{lang}</span><CopyCodeBtn code={codeLines.join('\n')}/></div>
    ...
    if (next.match(/^=+$/) && line.trim()) {
      elements.push(<h1 key={key++} className="md-h1">{inlineMd(line.trim())}</h1>);
      i += 2; continue;
    }
    if (next.match(/^-+$/) && line.trim() && !line.startsWith('-')) {
      elements.push(<h2 key={key++} className="md-h2">{inlineMd(line.trim())}</h2>);
      i += 2; continue;
    }
    ...
    if (hm) {
      ...
      const cls = ['md-h1','md-h2','md-h3','md-h3','md-h3','md-h3'][lvl-1];
      elements.push(<div key={key++} className={cls}>{inlineMd(hm[2])}</div>);
      i++; continue;
    }

    // Unordered list — handle -, *, + with any amount of trailing spaces
    if (line.match(/^[-*+]\s+\S/)) {
      const items = [];
      while (i < lines.length && lines[i].match(/^[-*+]\s+\S/)) {
        items.push(<li key={i}>{inlineMd(lines[i].replace(/^[-*+]\s+/, ''))}</li>);
        i++;
      }
    ...
    if (line.match(/^\d+[.)]\s+/)) {
      ...
      while (i < lines.length && lines[i].match(/^\d+[.)]\s+/)) {
        items.push(<li key={i}>{inlineMd(lines[i].replace(/^\d+[.)]\s+/, ''))}</li>);
        i++;
      }
    ...
    // Blockquote
    if (line.startsWith('> ')) {
      elements.push(<blockquote key={key++} className="md-bq">{inlineMd(line.slice(2))}</blockquote>);
      i++; continue;
    }
    ...
    // Empty line
    if (line.trim() === '') { elements.push(<div key={key++} className="md-br"/>); i++; continue; }

    // Paragraph
    elements.push(<p key={key++} className="md-p">{inlineMd(line)}</p>);
    i++;
  }
  return elements;
}
...
function inlineMd(text) {
  ...
  while (s.length) {
    // inline code
    const ic = s.match(/^([\s\S]*?)`([^`]+)`([\s\S]*)$/);
    // bold
    const bl = s.match(/^([\s\S]*?)\*\*(.+?)\*\*([\s\S]*)$/);
    // italic
    const it = s.match(/^([\s\S]*?)(?<!\*)\*(?!\*)(.+?)(?<!\*)\*(?!\*)([\s\S]*)$/);
    // link
    const lk = s.match(/^([\s\S]*?)\[([^\]]+)\]\(([^)]+)\)([\s\S]*)$/);

    const candidates = [
      ic && { type:'ic', pre:ic[1], mid:ic[2], rest:ic[3] },
      bl && { type:'bl', pre:bl[1], mid:bl[2], rest:bl[3] },
      it && { type:'it', pre:it[1], mid:it[2], rest:it[3] },
      lk && { type:'lk', pre:lk[1], mid:lk[2], href:lk[3], rest:lk[4] },
    ].filter(Boolean);

    if (!candidates.length) { tokens.push(<span key={k++}>{s}</span>); break; }
    const best = candidates.reduce((a,b) => a.pre.length <= b.pre.length ? a : b);
    if (best.pre) tokens.push(<span key={k++}>{best.pre}</span>);
    if (best.type==='ic') tokens.push(<code key={k++} className="md-ic">{best.mid}</code>);
    else if (best.type==='bl') tokens.push(<strong key={k++}>{best.mid}</strong>);
    else if (best.type==='it') tokens.push(<em key={k++}>{best.mid}</em>);
    else if (best.type==='lk') tokens.push(<a key={k++} href={best.href} target="_blank" rel="noreferrer" className="md-a">{best.mid}</a>);
    s = best.rest;
  }
  return tokens;
}
...
function CopyCodeBtn({code}) {
  ...
  return <button className={`md-copy ${ok?'ok':''}`} onClick={()=>{navigator.clipboard.writeText(code).then(()=>{setOk(true);setTimeout(()=>setOk(false),1800);});}}>{ok?'✓ copied':'copy'}</button>;
}

function MdMessage({text}) {
  ...
  const send = useCallback(async overrideText=>{
    ...
    try{
      ...
      if(api.type==="google"){
        ...
        const res=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${api.model}:generateContent?key=${key}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contents:msgs.filter(m=>m.role!=="system").map(m=>({role:m.role==="assistant"?"model":"user",parts:[{text:typeof m.content==="string"?m.content:m.content.map(p=>p.text||"").join("")}]})),systemInstruction:{parts:[{text:msgs.find(m=>m.role==="system")?.content||sys}]},generationConfig:{maxOutputTokens:settings.maxTokens,temperature:settings.temperature}})});
              ...
              {messages.map((msg,i)=>(
                <div key={i} className={`mrow ${msg.role}`}>
                  ...
                  <div className="mbbl">
                    ...
                    {msg.role==="assistant"?<MdMessage text={msg.displayText}/>:msg.displayText}
                  </div>
                  {msg.role==="assistant"&&(
          ...
          <div className="inp-box">
            ...
            <div className="inr">
              ...
              <div className="inp-modes">
                {curApi.supportsThinking&&<button className={`mode-pill ${adaptive?"on":""}`} onClick={()=>setAdaptive(v=>!v)} title="Deep thinking mode">🧠 Think</button>}
                <button className={`mode-pill srch ${searchMode?"on":""}`} onClick={()=>setSearchMode(v=>!v)} title="Web search mode">🔍 Search</button>
              </div>
