...
');
    ...
    const h2 = line.match(/^## (.+)/);
    const h1 = line.match(/^# (.+)/);
    if (h1) { elements.push(<h1 key={key++} className="md-h1">{inlineMarkdown(h1[1])}</h1>); i++; continue; }
    if (h2) { elements.push(<h2 key={key++} className="md-h2">{inlineMarkdown(h2[1])}</h2>); i++; continue; }
    if (h3) { elements.push(<h3 key={key++} className="md-h3">{inlineMarkdown(h3[1])}</h3>); i++; continue; }
    ...
    // Blockquote
    if (line.startsWith('> ')) {
      elements.push(<blockquote key={key++} className="md-bq">{inlineMarkdown(line.slice(2))}</blockquote>);
      i++; continue;
    }

    // Empty line
    if (line.trim() === '') {
      elements.push(<div key={key++} className="md-br"/>);
      i++; continue;
    }

    // Regular paragraph
    elements.push(<p key={key++} className="md-p">{inlineMarkdown(line)}</p>);
    i++;
  }
...
function inlineMarkdown(text) {
  ...
  while (remaining.length > 0) {
    ...
    const matches = [
      codeMatch && {type:'code', pre:codeMatch[1], content:codeMatch[2], rest:codeMatch[3]},
      boldMatch && {type:'bold', pre:boldMatch[1], content:boldMatch[2], rest:boldMatch[3]},
      italicMatch && {type:'italic', pre:italicMatch[1], content:italicMatch[2], rest:italicMatch[3]},
      linkMatch && {type:'link', pre:linkMatch[1], content:linkMatch[2], href:linkMatch[3], rest:linkMatch[4]},
    ].filter(Boolean);
    ...
    const best = matches.reduce((a,b) => a.pre.length <= b.pre.length ? a : b);

    if (best.pre) parts.push(<span key={key++}>{best.pre}</span>);

    if (best.type === 'code') parts.push(<code key={key++} className="md-ic">{best.content}</code>);
    else if (best.type === 'bold') parts.push(<strong key={key++}>{best.content}</strong>);
    else if (best.type === 'italic') parts.push(<em key={key++}>{best.content}</em>);
    else if (best.type === 'link') parts.push(<a key={key++} href={best.href} target="_blank" rel="noreferrer" className="md-a">{best.content}</a>);
...
function CopyCodeBtn({code}) {
  ...
  const copy = () => { navigator.clipboard.writeText(code).then(()=>{setOk(true);setTimeout(()=>setOk(false),1800);}); };
  return <button className={`md-copy ${ok?'ok':''}`} onClick={copy}>{ok?'✓ copied':'copy'}</button>;
}
